...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" <<-EOSQL
  CREATE USER ${DAWA_USER} WITH PASSWORD '${DAWA_PASSWORD}';
  CREATE DATABASE ${DAWA_DB}
  GRANT ALL PRIVILEGES ON DATABASE ${DAWA_DB} TO ${DAWA_USER};
EOSQL

psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" -d "${DAWA_DB}" <<-EOSQL
   GRANT ALL ON SCHEMA public TO ${DAWA_USER};
EOSQL
